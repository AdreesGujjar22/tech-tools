  ...
  return (
    <div className="min-h-screen bg-[#0C142B] text-[#E2DFFF] pt-32 pb-24 px-6 md:px-12">
      <div className="max-w-[1280px] mx-auto">
        ...
        <div className="mb-12 border-b border-white/10 pb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-6" id="blog-header">
          <div>
            ...
            <h1 className="text-5xl lg:text-6xl font-bold font-sans tracking-tight text-white mb-4">
              The Craft Blog
            </h1>
            <p className="text-lg text-[#C7C4D8] max-w-2xl leading-relaxed">
          ...
          {isAdmin && (
            <Link
              to="/admin"
              className="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-indigo-50 font-bold transition-all flex items-center gap-2 shadow-lg hover:shadow-indigo-500/20"
            >
              <PlusCircle size={18} />
              Write New Post
            </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-12" id="filters-workspace">
          ...
          <div className="lg:col-span-2 relative">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-indigo-300/60">
              <Search size={18} />
            </span>
            <input
              type="text"
              ...
              className="w-full pl-12 pr-4 py-3.5 bg-slate-900/50 border border-slate-800 focus:border-indigo-500 rounded-xl focus:outline-none placeholder-[#C7C4D8]/50 text-white transition-all text-sm"
            />
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-slate-900/50 border border-slate-800 focus:border-indigo-500 rounded-xl focus:outline-none text-sm text-[#DAD7FF] transition-all appearance-none cursor-pointer"
            >
              <option value="All" className="bg-[#0C142B]">Category: All</option>
              {uniqueCategories.filter(c => c !== "All").map((cat) => (
                <option key={cat} value={cat} className="bg-[#0C142B]">Category: {cat}</option>
              ))}
            </select>
          </div>
          ...
          <div className="relative">
            ...
            <select
              ...
              className="w-full pl-11 pr-4 py-3.5 bg-slate-900/50 border border-slate-800 focus:border-indigo-500 rounded-xl focus:outline-none text-sm text-[#DAD7FF] transition-all appearance-none cursor-pointer"
            >
              <option value="All" className="bg-[#0C142B]">Tag: All</option>
              {uniqueTags.filter(t => t !== "All").map((tag) => (
                <option key={tag} value={tag} className="bg-[#0C142B]">Tag: {tag}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((n) => (
              <div key={n} className="bg-slate-900/30 border border-slate-800/50 rounded-2xl p-6 h-[400px] animate-pulse flex flex-col justify-between">
                <div>
          ...
          <div className="premium-card p-12 text-center rounded-2xl max-w-lg mx-auto mt-12 border border-dashed border-indigo-500/20" id="empty-state">
            <Layers className="mx-auto text-indigo-500/50 mb-4" size={48} />
            ...
            <button
              ...
              className="text-indigo-400 hover:text-indigo-300 font-bold transition-all text-sm underline"
            >
              Clear Filters
            </button>
          </div>
          ...
          <div className="space-y-16">
            ...
            {featuredPost && search === "" && selectedCategory === "All" && selectedTag === "All" && (
              <motion.div
                ...
                id="featured-lens"
              >
                <div className="lg:col-span-7 relative h-64 md:h-96 rounded-2xl overflow-hidden bg-slate-900/60 flex items-center justify-center">
                  {featuredPost.featuredImage ? (
                    ...
                    <div className="text-slate-800 font-mono text-9xl select-none uppercase font-extrabold rotate-3">
                      CRAFT
                    </div>
                  )}
                  <div className="absolute top-4 left-4 px-3.5 py-1.5 rounded-lg bg-indigo-600/90 backdrop-blur-md text-xs font-mono font-bold tracking-widest text-[#E2DFFF]">
                    FEATURED POST
                  </div>
                </div>
                ...
                <div className="lg:col-span-5 flex flex-col justify-between py-2">
                  <div>
                    <span className="text-indigo-400 font-mono text-xs uppercase tracking-wider mb-2 block">
                      {featuredPost.category}
                    </span>
                    <h2 className="text-3xl font-bold tracking-tight text-white mb-4 hover:text-indigo-300 transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                      {featuredPost.tags?.slice(0, 3).map((tag) => (
                        <span key={tag} className="px-2.5 py-1 rounded-md bg-slate-900/60 border border-slate-800 text-xs text-[#C7C4D8]">
                          #{tag}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-6">
                    <div className="flex items-center gap-4 text-xs font-mono text-[#C7C4D8]/80">
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 text-indigo-400 hover:text-indigo-300 font-bold text-sm transition-all"
                    >
                      Read Full Post
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-white mb-8 font-sans">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="blog-grid">
                {filteredBlogs.map((post) => (
                  ...
                  >
                    <div>
                      {/* Card Cover */}
                      <div className="relative h-48 bg-slate-900/60 overflow-hidden flex items-center justify-center">
                        {post.featuredImage ? (
                          <img
                          ...
                          <div className="text-slate-800 font-mono text-6xl uppercase font-bold select-none">
                            CRAFT
                          </div>
                        )}
                        <span className="absolute bottom-3 left-3 px-2 py-1 rounded-md bg-slate-950/80 backdrop-blur-sm text-[10px] font-mono tracking-widest text-[#DAD7FF]">
                          {post.category?.toUpperCase()}
                        </span>
                        {post.status === "draft" && (
                          <span className="absolute top-3 right-3 px-2 py-0.5 rounded bg-amber-500 text-slate-950 text-[10px] font-mono font-bold uppercase">
                            Draft
                          </span>
                        )}
                      </div>
                      ...
                      <div className="p-6">
                        ...
                        <p className="text-[#C7C4D8] text-xs leading-relaxed line-clamp-3 mb-4">
                          {post.excerpt || `${post.content.replace(/<[^>]*>/g, "").substring(0, 110)}...`}
                        </p>
                        
                        <div className="flex flex-wrap gap-1.5">
                          {post.tags?.slice(0, 3).map((tag) => (
                            <span key={tag} className="text-[10px] font-mono text-indigo-300 px-2 py-0.5 rounded bg-indigo-950/45 border border-indigo-900/30">
                              #{tag}
                            </span>
                          ))}
                        </div>
                    ...
                    {/* Meta info footer */}
                    <div className="px-6 pb-6 pt-4 border-t border-white/5 flex items-center justify-between text-[11px] font-mono text-[#C7C4D8]/80">
                      <span className="flex items-center gap-1">
                        <Calendar size={12} />
                        {post.createdAt}
                      </span>
