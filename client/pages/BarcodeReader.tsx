...
export default function BarcodeReader() {
  ...
  return (
    <div className="min-h-screen bg-transparent text-foreground">
      <SEO title={t("title")} description={t("description")} keywords="QR reader, barcode scanner, scan QR image, webcam barcode scanner" />
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 mb-5 rounded-full border border-[#10A968]/20 bg-[#F0F7F0]">
              ...
              <span className="text-[#10A968] text-xs font-semibold tracking-wider uppercase">{t("privacy")}</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight gradient-text">{t("title")}</h1>
            <p className="mt-4 text-[#4A6857]">{t("description")}</p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6 lg:gap-8 items-start">
            <section className="glass-card-dark rounded-[24px] p-5 sm:p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div><h2 className="font-bold text-lg text-[#1F3A26]">{t("upload")}</h2><p className="text-sm text-[#4A6857]">{t("uploadDescription")}</p></div>
              </div>
              <div onDrop={handleDrop} onDragOver={(event) => { event.preventDefault(); setIsDragging(true); }} onDragLeave={() => setIsDragging(false)} className={`relative min-h-72 rounded-2xl border-2 border-dashed transition-colors flex flex-col items-center justify-center text-center p-6 ${isDragging ? "border-[#10A968] bg-[#F0F7F0]" : "border-[#C5DCC9] bg-white/70"}`}>
                {previewUrl ? <img src={previewUrl} alt="Uploaded code preview" className="max-h-56 max-w-full object-contain rounded-xl" /> : <><div className="w-14 h-14 rounded-full bg-[#F0F7F0] text-[#10A968] flex items-center justify-center mb-4"><Upload className="w-6 h-6" /></div><p className="font-semibold text-[#2D4D35]">{t("drop")}</p><p className="mt-1 text-sm text-[#4A6857]">{t("selectFile")}</p></>}
                {isScanningImage && <div className="absolute inset-0 rounded-2xl bg-white/85 backdrop-blur-sm flex flex-col items-center justify-center gap-3 text-[#10A968]"><LoaderCircle className="w-8 h-8 animate-spin" /><span className="text-sm font-semibold">{t("reading")}</span></div>}
              </div>
              <input ref={inputRef} type="file" accept="image/png,image/jpeg" onChange={handleFileChange} className="hidden" />
              <div className="mt-4 flex gap-3"><button onClick={() => inputRef.current?.click()} disabled={isScanningImage} className="flex-1 py-3 rounded-xl brand-gradient text-white text-sm font-semibold inline-flex justify-center items-center gap-2 disabled:opacity-50"><FileImage className="w-4 h-4" />{t("chooseImage")}</button>{previewUrl && <button onClick={clearScan} className="px-4 py-3 rounded-xl border border-[#C5DCC9] bg-white text-[#2D4D35]" aria-label="Clear image"><RotateCcw className="w-4 h-4" /></button>}</div>
            </section>

            <section className="glass-card-dark rounded-[24px] p-5 sm:p-8">
              <div className="flex items-center gap-3 mb-6"><div className="w-11 h-11 rounded-2xl bg-[#E8F5ED] text-[#10A968] flex items-center justify-center"><Camera className="w-5 h-5" /></div><div><h2 className="font-bold text-lg text-[#1F3A26]">{t("camera")}</h2><p className="text-sm text-[#4A6857]">{t("cameraDescription")}</p></div></div>
              <div className="relative min-h-72 overflow-hidden rounded-2xl bg-[#183A27] flex items-center justify-center">
                ...
                {!isCameraActive && <div className="text-center text-white/80 px-6"><div className="w-14 h-14 mx-auto mb-4 rounded-full bg-white/10 flex items-center justify-center"><Barcode className="w-6 h-6" /></div><p className="font-semibold text-white">{t("ready")}</p><p className="mt-1 text-sm">{t("cameraPrivacy")}</p></div>}
                {isCameraActive && <><div className="absolute inset-8 sm:inset-12 border-2 border-[#69E18B] rounded-2xl pointer-events-none"><span className="absolute left-0 right-0 top-1/2 h-px bg-[#69E18B] shadow-[0_0_12px_2px_#69E18B] animate-pulse" /></div><span className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black/55 px-3 py-1.5 text-xs text-white">{t("scanning")}</span></>}
              </div>
              <button onClick={isCameraActive ? stopCamera : startCamera} disabled={false} className={`mt-4 w-full py-3 rounded-xl text-sm font-semibold inline-flex justify-center items-center gap-2 transition-colors disabled:opacity-50 ${isCameraActive ? "border border-[#C5DCC9] bg-white text-[#2D4D35]" : "brand-gradient text-white"}`}>{isCameraActive ? <><Square className="w-4 h-4 fill-current" />{t("stopCamera")}</> : <><Camera className="w-4 h-4" />{t("startCamera")}</>}</button>
            </section>
          </div>

          <section className="mt-6 glass-card-dark rounded-[24px] p-5 sm:p-7 max-w-4xl mx-auto">
            <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-xl flex items-center justify-center ${result ? "bg-[#E8F5ED] text-[#10A968]" : error ? "bg-red-50 text-red-500" : "bg-[#E8F0E8] text-[#4A6857]"}`}>{result ? <CheckCircle2 className="w-5 h-5" /> : error ? <XCircle className="w-5 h-5" /> : <Clipboard className="w-5 h-5" />}</div><div><h2 className="font-bold text-[#1F3A26]">{t("result")}</h2><p className="text-sm text-[#4A6857]">{result ? `${result.format.replace(/_/g, " ")} detected` : error || t("emptyResult")}</p></div></div>{result && <button onClick={copyResult} className="px-4 py-2 rounded-xl bg-[#E8F5ED] text-[#176C3B] text-sm font-semibold inline-flex items-center gap-2">{copied ? <CheckCircle2 className="w-4 h-4" /> : <Copy className="w-4 h-4" />}{copied ? "Copied" : t("copyResult")}</button>}</div>
