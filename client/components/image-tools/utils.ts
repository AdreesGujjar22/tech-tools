...
/**
 * Log an image tool execution event to the Firebase analytics tracker.
 * Fully non-PII, keeping file statistics and speed telemetry clean and compliant.
 */
export async function logImageToolUsage(
  toolId: string,
  fileName: string,
  fileSize: number,
  success: boolean,
 ...
 */
export async function checkImageToolEnabled(toolId: string): Promise<boolean> {
  try {
    const docRef = doc(db, "image_tools_settings", toolId);
    ...
    console.error(`Error checking image tool setting for ${toolId}:`, err);
  }
 ...
 */
export async function setImageToolEnabled(toolId: string, enabled: boolean): Promise<void> {
  try {
    const docRef = doc(db, "image_tools_settings", toolId);
    await setDoc(docRef, {
      toolId,
    ...
    console.error(`Error saving setting for image tool: ${toolId}`, err);
    throw err;
  }
}
